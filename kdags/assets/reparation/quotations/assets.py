...
def mutate_so_quotations(context: dg.AssetExecutionContext, raw_so_quotations: pl.DataFrame):
    ...
    q_df = (
        ...
        .with_columns(
            ...
            file_name=pl.col("az_path").str.extract(r"/([^/]+)$", group_index=1),
            ...
            quotation_version=pl.col("az_path").str.extract(r"_V(\d+)_", group_index=1).cast(pl.Int64),
        )
        .with_columns(ppto_number=pl.col("file_name").str.extract(r"PPTO_(\d+)_V", group_index=1).cast(pl.Int64))
    )
    ...
    q_df = (
        q_df.with_columns(
            ...
            count_per_group=pl.col("file_name")
            .count()
            .over(["service_order", "quotation_version"])
        )
        .filter(
    ...
    df = (
        raw_so_quotations.rename({"quotation_amount": "repair_cost"})
        .drop("quotation_version")
        .with_columns(
            ...
            quotation_dt=pl.col("quotation_dt").str.to_datetime("%d %B %Y %H:%M:%S"),
        )
        .join(q_df, on=["service_order", "file_name"], how="left", validate="m:1")
    )
